...
int main()
{
	...
	wchar_t* target = L"C:\\Users\\jangh\\Documents\\GitHubVisualStudio\\bob_dbg\\x64\\Debug\\debuggee.exe";
		...
		case EXCEPTION_DEBUG_EVENT:
		{
			...
			case EXCEPTION_BREAKPOINT:
				if (true != dbge->_initial_bp_hit)
				{
					...
					bp_ret = set_single_step(&dbge->_bp_param);
					if (true != bp_ret)
					{
						_ASSERTE(true == bp_ret);
						break;
				...
				else
				{
					...
					_pause;
					bool bp_ret = set_single_step(&dbge->_bp_param);
					if (true != bp_ret)
					{
						_ASSERTE(true == bp_ret);
						break;
